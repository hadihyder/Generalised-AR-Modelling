...
import GARMListView from './component/GARMListView';
...
// AR Scene that's rendered on the MAIN Screen. App state changes propagate to GARM.js via redux 
...
export class App extends Component {
  ...
  // This render() function renders the AR Scene in <ViroARSceneNavigator> with the <ViroARScene> defined in GARM.js
        ...
        <View style={localStyles.flex}>
          ...
          {renderIf(this.props.currentScreen != UIConstants.SHOW_SHARE_SCREEN,
            <View style={localStyles.listView}>
              <GARMListView items={this._getListItems()} onPress={this._onListPressed} />
            </View>
            )}
  ...
  async requestAudioPermission() {
    try {
      const granted = await PermissionsAndroid.request(
        ...
        {
          'title': 'GARM Audio Permission',
          'message': 'GARM App needs to access your audio ' +
  ...
  async requestWriteAccessPermission() {
    try {
      const granted = await PermissionsAndroid.request(
        ...
        {
          'title': 'GARM Audio Permission',
          'message': 'GARM App needs to access your photos / videos ' +
...
_takeScreenshot() {
  ...
  this._arNavigator._takeScreenshot("GARM_still_" + this.state.screenshot_count, false).then((retDict)=>{
    if (!retDict.success) {
...
_startRecording() {
  ...
  this._arNavigator._startVideoRecording("GARM_video", false,
     (errorCode)=>{
